...
static void FLASH_OB_PCROPConfig(uint32_t PCROConfigRDP, uint32_t PCROPStartAddr, uint32_t PCROPEndAddr,
                                 uint32_t Banks);
...
static void FLASH_OB_SecureAreaConfig(uint32_t SecureAreaConfig, uint32_t SecureAreaStartAddr,
                                      uint32_t SecureAreaEndAddr, uint32_t Banks);
static void FLASH_OB_GetSecureArea(uint32_t* SecureAreaConfig, uint32_t* SecureAreaStartAddr,
    ...
    if (status == HAL_OK)
    {
        if (pCRCInit->Bank == FLASH_BANK_1)
        {
            ...
            if (pCRCInit->TypeCRC == FLASH_CRC_SECTORS)
            {
                ...
                /* Select CRC sectors */
                for (sector_index = pCRCInit->Sector; sector_index < (pCRCInit->NbSectors + pCRCInit->Sector);
                     sector_index++)
                {
...
void HAL_FLASHEx_GetEccInfo(FLASH_EccInfoTypeDef* pData)
{
    ...
    if (errorflag != 0U)
    {
        ...
        pData->Address = ((((FLASH->ECC_FA1 & FLASH_ECC_FA_FAIL_ECC_ADDR)) * FLASH_NB_32BITWORD_IN_FLASHWORD * 4) +
            FLASH_BANK1_BASE);
    }
#if defined (DUAL_BANK)
