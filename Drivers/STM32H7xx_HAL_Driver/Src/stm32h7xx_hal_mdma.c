                                           ...
                                           uint32_t Timeout)
{
    ...
    levelFlag = ((CompleteLevel == HAL_MDMA_FULL_TRANSFER)
                     ? MDMA_FLAG_CTC
                     : (CompleteLevel == HAL_MDMA_BUFFER_TRANSFER)
                     ? MDMA_FLAG_BFTC
                     : (CompleteLevel == HAL_MDMA_BLOCK_TRANSFER)
                     ? MDMA_FLAG_BT
                     : MDMA_FLAG_BRT);
...
static void MDMA_Init(MDMA_HandleTypeDef* hmdma)
{
    ...
    */
    ...
    {
        /*write new CBRUR Register value : destination repeat block offset */
        hmdma->Instance->CBRUR |= ((((uint32_t)hmdma->Init.DestBlockAddressOffset) & 0x0000FFFFU) <<
            MDMA_CBRUR_DUV_Pos);
    }
